...
import utils
from data import VerseFlowDataset
from model import FlowNet3D
from test import test_one_epoch, test_verse20, compute_test_metrics, get_color_array
...
def train(args, net, train_loader, val_loader, test_loader, textio):
    ...
    for epoch in range(args.epochs):
        ...
        if report_val_loss:
            wandb.log({'Train': train_losses, 'Validation': val_losses, 'epoch': epoch,
                       'val_loss': val_losses[args.sweep_target_loss], 'Test during training': test_loss})
            report_val_loss = False
        else:
            wandb.log({'Train': train_losses, 'Validation': val_losses, 'Test during training': test_loss, 'epoch': epoch})
...
def run_experiment(args):
    ...
    train_set = VerseFlowDataset(npoints=4096, root=args.dataset_path, mode="train", raycasted=args.use_raycasted_data,
                                 augment=not args.no_augmentation, data_seed=args.data_seed, test_id=args.test_id,
                                 occlude_data=args.occlude_data, occlude_ratio=args.occlude_ratio,
                                 max_rotation=args.max_rotation, num_folds=args.num_folds)
    ...
    val_set = VerseFlowDataset(npoints=4096, root=args.dataset_path, mode="val", raycasted=args.use_raycasted_data,
                               splits=train_set.spine_splits, occlude_data=args.occlude_data,
                               occlude_ratio=args.occlude_ratio, max_rotation=args.max_rotation,
                               augment_test=args.augment_test, num_folds=args.num_folds)
    ...
    test_set = VerseFlowDataset(npoints=4096, root=args.test_dataset_path, mode="test",
                                ...
                                splits=train_set.spine_splits, occlude_data=args.occlude_data,
                                occlude_ratio=args.occlude_ratio, max_rotation=args.max_rotation,
                                augment_test=args.augment_test, test_rotation_axis=args.test_rotation_axis,
                                test_rotation_degree=args.max_rotation, num_folds=args.num_folds)
    ...
    # test after training
    test_verse20(args, best_net, textio, spine_splits=train_set.spine_splits)
...
def main():
    ...
    if args.wandb_sweep_id is not None:
    ...
    elif args.run_cross_val:
        for num in range(args.num_folds):
            args_ = Namespace(**vars(args))
            args_ = utils.update_args(args_)
            wandb_name = f"{args_.wandb_name}_fold{num}"
            wandb.init(project='spine_flownet', config=args_, name=wandb_name)
            args_.data_seed = num
            run_experiment(args_)
            wandb.finish()
    else:
        ...
        if args.wandb_name != "":
            wandb.init(project='spine_flownet', config=args, name=args.wandb_name)
        else:
